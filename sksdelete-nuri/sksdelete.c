...
int main(int argc, char *argv[])
{
    ...
    /* add cert */
    /*
    if ( (wolfSSL_CTX_use_certificate_file(ctx, "ra-tls-cert/cert_pem.crt", SSL_FILETYPE_PEM)) != WOLFSSL_SUCCESS) {
        err_sys("wolfSSL_use_certificate_file error");
    }

    if ( (wolfSSL_CTX_use_PrivateKey_file(ctx, "ra-tls-cert/key_pem.pem", SSL_FILETYPE_PEM)) != WOLFSSL_SUCCESS) {
